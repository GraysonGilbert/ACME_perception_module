...
inline LetterboxMeta letterbox(const cv::Mat& src, cv::Mat& destination, const cv::Size& destination_size,
  ...
  float ratio = std::min(destination_size.width / (float)src.cols, destination_size.height / (float)src.rows);
  ...
  cv::copyMakeBorder(resized, destination, metadata.pad_top, metadata.pad_bottom, metadata.pad_left, metadata.pad_right,
...
inline cv::Mat unletterboxDepth(const cv::Mat& depth_destination, const LetterboxMeta& metadata) {
  ...
  cv::Rect region_of_interest(metadata.pad_left, metadata.pad_top,
               metadata.destination_size.width - (metadata.pad_left + metadata.pad_right),
               metadata.destination_size.height - (metadata.pad_top + metadata.pad_bottom));
