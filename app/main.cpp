...
int main(int argc, char **argv) {
	...
	for (size_t i = 0; i < res.boxes.size(); ++i) {
		...
		std::string name = (cls >= 0 && static_cast<size_t>(cls) < classes.size())
							 ? classes[cls]
							 : std::to_string(cls);
		std::cout << i << ": class=" << name << " id=" << cls
					<< " conf=" << conf << " box=[" << b.x << ", " << b.y
					<< ", " << b.width << ", " << b.height << "]\n";
	}
	...
	for (size_t i = 0; i < res.boxes.size(); ++i) {
		...
		cv::Scalar color = colors[cls >= 0 ? (cls % colors.size()) : (i % colors.size())];
		...
		cv::putText(annotated, label, cv::Point(x + 2, label_y - 2), fontFace, fontScale,
