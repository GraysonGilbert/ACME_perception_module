...
TEST(DepthProcessorTest, LoadModelIdeal) {
	DepthProcessor proc;
	DepthConfig cfg;
	cfg.model_path = std::string("app/config_files/depth_anything_vitb14_fixed.onnx");
...
TEST(DepthProcessorTest, ProcessAfterLoadModelReturnsDepthMaps) {
	...
	EXPECT_EQ(res.visualization.type() % 8, CV_8U) << "visualization should be 8-bit";
...
TEST(DepthProcessorTest, ColorizeProduces3ChannelImage) {
	...
	EXPECT_EQ(color.type() % 8, CV_8U) << "colorized output should be 8-bit image";
}
